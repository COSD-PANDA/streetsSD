var sqlBuilder = (function() {
  ...
  getLastQuarter = function() {
    ...
    var lastQuarterEndDate = moment(date)
      .subtract({
    ...
    switch (stringRef) {
      ...
      case 'work_scheduled':
        ...
        return (
          ...
          schedField +
          ') >= 7 ' +
          "THEN 'FY-' || EXTRACT (YEAR FROM " +
          schedField +
          ') + 1 ' +
          "ELSE 'FY-' || EXTRACT (YEAR FROM " +
          schedField +
          ') ' +
          'END'
        );
  ...
  mapAlias = function(table_alias, field_alias) {
    ...
    if (_.isUndefined(field_alias)) return tables[table_alias];
  ...
  getTableSQL = function(sqlKey) {
    ...
    if (sqlKey == 'oci-2015' || sqlKey == 'oci-2011') {
      ...
      SQL.from(mapAlias(alias), alias).join(
        tables.tswb,
        'tswb',
        alias + '.seg_id = tswb.seg_id',
      );
    } else {
  ...
  getDistanceSQL = function(sqlKey, config) {
    ...
    if (config.groupFieldSQL) groupField = config.groupFieldSQL;
    ...
    SQL.field(groupField, config.groupFieldAlias)
      .field(
        'SUM(' + mapAlias(config.tableAlias, config.lengthFieldAlias) + ')',
        'totalMiles',
      )
      .group(groupField);

    if (sqlKey == 'oci-2011') SQL.from(mapAlias('oci2011'), 'oci2011');
    else if (sqlKey == 'oci-2015') SQL.from(mapAlias('oci2015'), 'oci2015');
    else {
      // All others
    ...
    if (config.order) SQL.order(groupField, config.order == 'ASC');
